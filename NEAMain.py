...
def Main():  #main function to call 
    while 1:
        MainGame = Game() 
        MainGame.MainMenu()
        del MainGame  #the purpose of this is as long as the user does not quit the program, a new instance of the class can be made again in the event the user dies in the game
...
class Game(GameStats):  #inherits the GameStats class
    def __init__(self):
        ...
        self._textfont = pygame.font.Font(r'C:\Windows\Fonts\georgia.ttf', 50 ) #fonts for text
        self._symbol = pygame.font.Font(r'C:\Windows\Fonts\seguisym.ttf', 50)
    ...
    def Save(self): #save function to save player progress
        obslist=[]
        with open("file1", "wb") as file1:   #in this file, it saves the map, the current position of the player, the rooms traversed, player health,
            pickle.dump(self.Map,file1)      #avatar size and colour, the x and y position of the player, and the boss room element position
    ...
    def Load(self):
        try: #error handle in case if the file does not exist
            with open("file1","rb") as file1:
                ...
                map = pickle.load(file1)  #based on the order on which info has been dumped, the order that the variables get loaded will be the same 
    ...
    def RunGame(self,File=None,health=3,enemyHealthInc=0):  #runs the game, there are preset values incase changes occur during the game
        self.ObstacleGroup = pygame.sprite.Group() #initialises a sprite group for obstacles
        if File != None:  #if the file is not empty
            self.Map = File[0] #load respective values into the variables
            playerpos = File[1]
            obstaclelist = File[2]
            ...
            BossPos = File[5]
            currHealth = File[6]
            GameHud = Hud(self._screen,currHealth) #Hud to display health
            ...
            self.globalpos = playerpos  #attributes are assigned these values so that they can be accessed in the Save() function
            ...
            self.Map,BossPos = self.GenerateMap() #generates a map and the position of the boss room within the map
            self.BossPos = BossPos
            ...
            for i,value in enumerate(self.Map):  #iterates the entire self.Map to add a separate obstacle,
                for j,val in enumerate(value):  
                    ...
                    for _ in range(0,randomobs):
                        ...
                        if (i,j) == BossPos: #if the position is the boss room
                            ...
                            self.randomxcenter2 = random.randint(1300,1500)
        ...
        IsBoss = False
        iFrames_time = pygame.USEREVENT+0  #short for Invincibility Frames, this is a custom event id.
        iFrames= pygame.event.Event(iFrames_time) #a custom event to be used later on
        ...
        while running:  
            ...
            if EnemyInRoom == 0:  #if there are no enemies in the room, then check the player position on the self.Map and see if the adjacent rooms are passable or not.
                ...
                if BossDefeated != 0: #if the boss is defeaeted on this floor
                    if (playerpos[0],playerpos[1]) == self.BossPos: #and if the current position of the player is indeed the boss room
                ...
                for enemy in self.enemies: 
                    if enemy.state == 1: #for ranged enemies, as they can also attack using projectiles in intervals of one second 
                        if enemy.cooldown == 0: #if the cooldown is 0
                            intervals = runningtime +1000 #set the interval time to 1 second ahead of the current running time (time is measured in milliseconds)
                    ...
                    for enemyprojectile in enemy.projectilegroup:
                        ...
                        if len(collision)!=0 and self.player.state == 1: #if the attack projectile collides, -1 to enemy health 
                            ...
                            if GameHud.health >0: #if the player's health is greater than 0,
                                self.Health = GameHud.health #set the health value 
                                newtime = runningtime +1500 #add 1.5 seconds to the current running time
                                pygame.event.post(iFrames) #and force a event to the event queue
                                check = 1 
            ...
            for event in pygame.event.get(): #gets the event
                if event.type == pygame.QUIT: # Did the user click the window close button?
                    running = 0
                    sys.exit()  
                if event == iFrames:  #if the player gets hit by the enemy
    ...
    def LevelBetween(self,Health=3): #the intermission betweeen levels, with the default parameter of the health being 3, any changes to this parameter
        self.Level+=1                #will be passed through and carried on to the next level
        if self.rooms < self.MaxRooms: #there is limit to how many rooms can exist as the algorithm to generate rooms may reach the recursive depth if too many rooms are added
            self.rooms+=random.randint(1,3)
        if self.enemyRooms < self.MaxEnemy and self.enemyRooms < self.rooms: #same goes with the maximum enemy rooms, whilst a similar algorithm, it may not break out of the loop if there are too many enemy rooms to be added
            self.enemyRooms+= random.randint(1,2)
        ...
        while running:
            ...
            if self.Level == 1: #if the level is currently 1
                ...
                if Play.collidepoint(mousex,mousey) and click == 1:
                    running = 0
                    self.rooms += random.randint(1,5) #otherwise, a random amount of rooms will be added
                    self.enemyRooms+= random.randint(0,3)
    ...
    def BossOrNot(self,input): #simple function to check whether if a room is a boss or not
        if input == 'B':
            return True
        return False       
    
    def Controls(self): #controls for the player 
        running = 1
        ...
        while running:
            ...
            if cont.collidepoint(mousex,mousey) and event.type == pygame.MOUSEBUTTONDOWN: #if the mouse position collides at the text and the user presses the mouse button
    ...
    def MainMenu(self):
        ...
        while running:
            ...
            Play = pygame.draw.rect(self._screen, self._White,(900,200,100,100))
            self._screen.blit(self._textfont.render("Play",1, self._Black), (900,200))
            Exit = pygame.draw.rect(self._screen, self._White,(900,800,100,100))
            self._screen.blit(self._textfont.render("Exit",1, self._Black), (900,800))
            Load = pygame.draw.rect(self._screen, self._White,(900,400,100,100))
            self._screen.blit(self._textfont.render("Load",1, self._Black), (900,400))
            Erase = pygame.draw.rect(self._screen, self._White,(840,600,240,100))
            self._screen.blit(self._textfont.render("Erase Save",1, self._Black), (840,600))
